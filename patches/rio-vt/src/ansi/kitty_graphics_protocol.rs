...
fn create_graphic_data(cmd: &KittyGraphicsCommand) -> Result<GraphicData, GraphicError> {
    ...
    let raw_data = match cmd.medium {
        ...
        TransmissionMedium::SharedMemory => {
            // mobile-coder patch: Android has no POSIX shared memory - bionic
            // provides neither shm_open nor shm_unlink, so this did not compile.
            // A program in the proot guest could not share memory with the app
            // anyway, so report the medium as unavailable.
            #[cfg(target_os = "android")]
            {
                debug!("Shared memory transmission is not supported on Android");
                return Err(GraphicError::FileNotFound);
            }
