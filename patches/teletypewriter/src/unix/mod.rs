...
// mobile-coder patch: Android. bionic's ioctl takes a c_int request.
...
// mobile-coder patch: bionic has openpty/forkpty in libc (API 23+) and ships
// no libutil, so linking against it fails on Android.
...
pub fn create_termp(utf8: bool) -> libc::termios {
    ...
    // mobile-coder patch: Android. bionic's termios is the Linux kernel layout
    // without glibc's trailing c_ispeed/c_ospeed, so neither the glibc nor the
    // musl arm above matches it and `term` was left undefined on Android.
    #[cfg(target_os = "android")]
    let mut term = libc::termios {
        c_iflag: libc::ICRNL | libc::IXON | libc::IXANY | libc::IMAXBEL | libc::BRKINT,
        c_oflag: libc::OPOST | libc::ONLCR,
