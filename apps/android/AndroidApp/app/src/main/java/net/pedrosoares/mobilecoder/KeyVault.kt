...
object KeyVault {
    ...
    fun store(context: Context, plaintext: String) {
        ...
        file(context).writeBytes(cipher.iv + sealed)
        Log.i(TAG, "api key stored (${sealed.size} bytes sealed)")
    }
    ...
    fun load(context: Context): String? {
        ...
        return try {
            ...
            if (blob.size <= IV_BYTES) {
                Log.w(TAG, "stored api key is truncated; ignoring")
                return null
            }
